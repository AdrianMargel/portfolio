...
let theme;
{
	...
	// let greyStep=(a)=>{
	// 	// a = (-infinity,infinity)
	// 	let shadow=new Color("#E5416E").sclCol(1);
	// 	// shadow=new Color("#000000");
	// 	let base=new Color("#FC8420");
	// 	base.subCol(shadow).limCol();
	...
	// };
	let greyStep=(a)=>{
		let base=new Color("#E5416E").subCol(new Color("#FC8420"));
		return base.apply((v)=>v*sigmoid(-a-2)).addCol(new Color("#FC8420")).addCol(new Color("#202020")).limCol();
	};
	// let greyStep=(a)=>{
	...
	let boxShadowStep=(a)=>{
		return "box-shadow: 0 0 "+genericStep(a,80,0,30)+"px #00000040;";
	}
...
defineElm(HomePage,scss`&{
	...
	>.main{
		...
		min-height: 400px;
		background-color: ${theme.color.backWhite};
		>div{
			${theme.center}
			flex-direction: column;
			h2{
...
defineElm(AboutPage,scss`&{
	...
	h3{
		background-color: ${theme.color.greyStep(-3)};
		text-align: center;
		${theme.font.primary}
		${theme.font.fontSizeStep(0)};
		line-height: 1.5;
...
defineElm(VolumeInput,scss`&{
	...
	>.volume{
		...
		min-height:50px;
		box-sizing: border-box;
		background-color: ${theme.color.greyStep2(1)};
		border:2px solid ${theme.color.greyStep2(1)};
		border-radius:25px;
		${theme.center}
		transition: border 0.2s;
