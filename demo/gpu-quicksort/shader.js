...
function boxSize(length,opts){
	let components=opts?.components??1;
	let arrLeng=length/components;
	let width=opts?.width??ceil(sqrt(arrLeng));
	let height=ceil(arrLeng/width);//TODO: support enforced opts height
	return [width,height];
}
function boxArray(arr,padding=0,opts){
	let components=opts?.components??1;
	let size=boxSize(arr.length,opts);
	let boxLeng=size[0]*size[1]*components;

	let arr2=[...arr];
	let padLeng=boxLeng-arr2.length;
	arr2.length=boxLeng;
	if(padLeng>0){
