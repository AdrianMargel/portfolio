...
// Get the value at a given index in the cascade
vec4 getAt(Level level,Cascade cascade,Probe probe,Ray ray){
	ray.idx=clamp(ray.idx,0.,probe.leng-1.);
...
// Merges two segments into a smaller segment which is the overlap between both.
// Supports the case where a segment angles loop around at TAU.
...
// Transforms a segment with an offset and scale
vec3 transformSegment(vec3 seg,float offset,float scale){
	return vec3(
		nrmAngPI(seg.x-offset)/scale,
		nrmAngPI(seg.y-offset)/scale,
		seg.z
	);
}
// Takes a segment and converts it into an equation in the form (ax+b) as a vec2(a,b)
...
// Integrate (ax+b)*(cx+d) from 0 to 1
...
// Integrate (ax+b)*(cx+d)*(ex+f) from 0 to 1
...
// Integrates the overlap of 3 segments including slope TODO:swap
...
// Integrates the overlap of 3 segments without slope TODO:swap
float integral2(vec3 segA,vec3 segB,vec3 segC){
	vec3 seg=mergeSegments(mergeSegments(segA,segB),segC);
	float start=seg.x;
	float end=seg.y;
	float dist=max(end-start,0.);
...
// Integrates the overlap of 2 segments including slope
...
// Gets the cone for a probe index
vec3 getProbeCone2(float idx,vec2 origin,Cascade cascade,Level level){
	float mid=angle(
		origin,
		newProbe(idx,cascade,level).pos
	);
	mid=nrmAngTAU(mid);
	return vec3(mid-1.,mid,mid+1.);
}
// Gets the cone for a ray index
...
vec3 getRayCone2(float idx,Probe probe){
	float mid=newRay(idx,probe).ang;
	float start=newRay(idx-.5,probe).ang;
	float end=newRay(idx+.5,probe).ang;
	return vec3(start,mid,end);
}
...
// Get the start and end probe indexes for a segment.
// The start index will always be smaller. Indexes may be out of range,
// use a modulo to correct this. 
...
// Get the start and end ray indexes for a segment.
// The start index will always be smaller. Indexes may be out of range,
// use clamp to correct this.
			...
			glsl`#version 300 es
				...
				void main(){
					...
					vec4 cascadeVal;
					{
						...
						// VIEW
						{
							vec3 viewCone=getRayCone2(rayIdx,sourceProbe);
							vec3 viewSeg=vec3(viewCone.xz,0.);

							vec3 probeCone=getRayCone2(rayIdx,sourceProbe);
							vec3 probeSeg=vec3(probeCone.xz,0.);
							vec3 rayCone=getRayCone2(rayIdx,sourceProbe);
							vec3 raySeg=vec3(rayCone.xz,0.);
