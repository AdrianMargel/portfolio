	...
	constructor(){
		...
		this.define(html`
			...
			${new Surface(html`
				<h2>Explanation</h2>
				<p>
					To solve this problem I decided to come at it from a unique perspective - voxels. Most computer graphics use triangle mesh to describe complex scenes. Triangle mesh has many great advantages, however it does not lend itself well to ray tracing. In fact many approaches to ray trace triangle meshes involve wrapping the mesh in a Sparse Voxel Octree (SPO) to accelerate performance. I decided for this project to limit the entire scene to voxels, both for simplicity and performance. It is worth noting that this does not exclude use of triangle mesh, it is merely beyond the scope of this project.
				</p>
				<h3>Core</h3>
				<p>
					The core of this project is a ray tracing engine optimized for SPOs. The algorithm itself to ray trace a SPO is pretty straight-forward, its implementation however is not. WebGL is intended for rendering triangle meshes with textures and shaders. In order to use it for arbitrary ray tracing requires very creative use of its features.
				</p>
				<p>
